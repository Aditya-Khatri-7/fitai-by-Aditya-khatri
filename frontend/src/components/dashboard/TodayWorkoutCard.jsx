...
import { reshuffleWorkout as reshuffleAction } from '../../redux/slices/workoutSlice';
          ...
          >
            <div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="font-bold text-[var(--text-tertiary)]">#0{idx + 1}</span>
                {ex.isAISwapped && (
                  <span className="px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 font-bold text-[9px]">
                    AI Swapped
                  </span>
                )}
              </div>
              <h4 className="text-xs font-bold text-[var(--text-primary)] mt-1 line-clamp-1">{ex.name}</h4>
