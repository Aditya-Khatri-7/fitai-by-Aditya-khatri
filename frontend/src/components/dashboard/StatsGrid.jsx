...
export function StatsGrid({ metrics, streak }) {
  ...
  return (
    <div className={`grid grid-cols-1 gap-4 ${isCompact ? '' : 'sm:grid-cols-2 lg:grid-cols-4'}`}>
      {stats.map((item, idx) => {
        const Icon = item.icon;
        return (
          <div key={idx} className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-xl flex items-center gap-4 min-w-0">
            <div className={`w-12 h-12 rounded-xl ${item.bg} flex items-center justify-center shrink-0`}>
              <Icon className={`w-6 h-6 ${item.color}`} />
            </div>
            <div className="min-w-0">
              <p className="text-xs text-[var(--text-secondary)] font-medium break-words">{item.label}</p>
              <h3 className="text-lg font-extrabold text-[var(--text-primary)] mt-0.5 tracking-tight font-mono break-words">{item.value}</h3>
              <p className="text-[10px] text-[var(--text-tertiary)] mt-0.5 break-words">{item.target}</p>
            </div>
          </div>
        );
      })}
