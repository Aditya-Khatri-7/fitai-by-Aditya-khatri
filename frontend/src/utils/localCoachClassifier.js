/**
 * Client-Side Local Coach Classifier (Instant, Zero-Network, Zero-Token)
 * Classifies 100% of standard commands locally: Themes (all 14), Meals (Breakfast, Lunch, Dinner, Snacks),
 * Workouts (Muscle Merges), Wearable Sync, Profile Avatar.
 */
...
const THEME_SYNONYMS = {
  midnight_carbon: ['default', 'neutral', 'standard', 'slate', 'dark'],
  obsidian_blood: ['intense', 'aggressive', 'power', 'fierce', 'blood', 'angry', 'red'],
  void_purple: ['creative', 'mysterious', 'artsy', 'purple', 'imaginative'],
  forest_deep: ['natural', 'nature', 'earthy', 'green', 'grounded', 'organic', 'forest'],
  ocean_abyss: ['cool', 'calm', 'focused', 'ocean', 'cyan', 'soothing', 'chill', 'relax', 'relaxing', 'peaceful', 'mood', 'blue'],
  burnt_circuit: ['fiery', 'energetic', 'amber', 'circuit', 'hype'],
  neon_noir: ['edgy', 'vibrant', 'neon', 'club', 'magenta'],
  clinical_white: ['clean', 'minimal', 'professional', 'clinical', 'white', 'light', 'bright'],
...
function matchTheme(text) {
  ...
  if (!best) {
    ...
    else if (wordBoundary(text, 'calm') || wordBoundary(text, 'blue') || wordBoundary(text, 'relax')) best = 'ocean_abyss';
    else best = 'clinical_white';
  }
  return best;
}
 ...
 * lunch instead of one diet flag getting applied to every meal in the message.
 */
function extractPerSlotDiet(clean) {
  ...
  const bestPerSlotOccurrence = new Map();
...
function buildMeal(mealType, dietTag, wantsLighter) {
  const template = MEAL_TEMPLATES[mealType]?.[dietTag] || MEAL_TEMPLATES[mealType]?.['vegetarian'];
...
export function classifyLocallyClient(message) {
  const clean = (message || '').toLowerCase().replace(/[^a-z0-9\s-]/g, ' ');

  // THEME
  if (wordBoundary(clean, 'theme') || wordBoundary(clean, 'mood') || wordBoundary(clean, 'vibe') || wordBoundary(clean, 'style') || wordBoundary(clean, 'color')) {
    const themeId = matchTheme(clean);
    const themeName = themeId.replace(/_/g, ' ').toUpperCase();
    return {
      replyText: `Synthesized theme modification: Switching UI styling to ${themeName}!`,
      datasetSuggestions: [{ id: 'theme_1', title: themeName, details: 'GPU-Accelerated CSS Token Customization' }],
      proposedAction: { type: 'CHANGE_THEME', payload: themeId, description: `Switch theme to ${themeName}` }
    };
  }
  ...
  if (wordBoundary(clean, 'remove') && (wordBoundary(clean, 'snack') || wordBoundary(clean, 'snacks'))) {
    return {
      replyText: "Removing snacks from today's meal plan and recalculating your macro totals.",
  ...
  if ((wordBoundary(clean, 'add') || wordBoundary(clean, 'want')) && (wordBoundary(clean, 'snack') || wordBoundary(clean, 'snacks'))) {
    return {
      replyText: "Adding a healthy snack to today's meal plan.",
  ...
  if (wordBoundary(clean, 'wearable') || wordBoundary(clean, 'fitbit') || wordBoundary(clean, 'garmin') || wordBoundary(clean, 'sync')) {
    return {
      replyText: 'Opening Fitbit & Apple Health telemetry sync panel.',
      proposedAction: { type: 'SYNC_WEARABLE', payload: null, description: 'Open wearable telemetry sync' }
    };
  }
  ...
  if (wordBoundary(clean, 'avatar') || wordBoundary(clean, 'profile picture') || wordBoundary(clean, 'photo')) {
    return {
      replyText: 'Opening profile settings with avatar presets.',
  ...
  if ((workoutTriggered || groups.length > 0) && groups.length > 0) {
    const title = `${groups.map(g => g.toUpperCase()).join(' & ')} Specialization Split`;
    return {
      replyText: `Compiling a ${title} for today.`,
  ...
  if (slotsHit.length > 0) {
    ...
    const meals = slotsHit
      .map(slot => buildMeal(slot, slotDiets[slot] || 'vegetarian', wantsLighter))
      .filter(Boolean);

    if (meals.length === 1) {
      return {
        replyText: `Updating ${meals[0].type} to ${meals[0].name} (${meals[0].totalCalories} kcal).`,
        datasetSuggestions: [{ id: 'm_1', title: meals[0].name, details: `${meals[0].totalCalories} kcal` }],
    ...
    if (meals.length > 1) {
      return {
        ...
        datasetSuggestions: meals.map((m, i) => ({ id: `m_${i}`, title: m.name, details: `${m.totalCalories} kcal` })),
  ...
  // GENERAL FALLBACK
  return {
    replyText: `I analyzed your request "${message}". I can update workouts, meal plans (breakfast, lunch, dinner, snacks), theme styles, or sync wearable metrics. What would you like to do?`,
    datasetSuggestions: [{ id: 'gen_1', title: 'Adaptive Fitness & Nutrition Optimization', details: 'Full System Control Active' }]
  };
}
